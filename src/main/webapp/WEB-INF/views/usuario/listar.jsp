	...
	<div class="container">
		...
		<div class="row center">
			...
			<div class="input-field left">
				...
				href="${s:mvcUrl('novoUsuario').build() }"><i class="material-icons left">add</i>adicionar</a>
			</div>
			...
			<div class="row">
						...
						<tbody>
							<c:forEach items="${listaPaginada.content}" var="usuario">
								<tr>
									...
									<td><img src="/img/default_avatar.png" width=30 alt="Avatar do Usuário"></td>
									<td>${usuario.login}</td>
									<td>${usuario.email}</td>
				...
				<td class="center-align"><a class="waves-effect waves-light btn-small yellow"
						href="${s:mvcUrl('visualizarUsuario').arg(0, usuario.idLogin).build() }" title="Visualizar"><i
							class="material-icons yellow-text text-darken-3">remove_red_eye</i></a> <a
						class="waves-effect waves-light btn-small blue"
						href="${s:mvcUrl('editarUsuario').arg(0, usuario.idLogin).build() }" title="Editar"><i
					...
					<button href="#modalExcluir" class="modal-excluir modal-trigger waves-effect waves-light btn-small red"
						data-descr="${usuario.login }" data-tabela="usuarios" data-id="${usuario.idLogin }" title="Excluir">
						<i class="material-icons red-text text-darken-3">delete</i>
						<f:form action="${s:mvcUrl('apagarUsuario').arg(0, usuario.idLogin).build() }" method="POST">
						</f:form>
					</button></td>
				</tr>
				</c:forEach>
